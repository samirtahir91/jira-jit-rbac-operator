...
.PHONY: test-e2e
test-e2e: kind-delete manifests generate fmt vet kind-create ## Run the e2e tests. Expected an isolated environment using Kind.
	@command -v kind >/dev/null 2>&1 || { \
		echo "Kind is not installed. Please install Kind manually."; \
		exit 1; \
	}
	@kind get clusters | grep -q 'jira-jit-rbac-operator' || { \
...
docker-buildx: ## Build and push docker image for the manager for cross-platform support
	...
	- $(CONTAINER_TOOL) buildx create --name jira-jit-rbac-operator-builder
	$(CONTAINER_TOOL) buildx use jira-jit-rbac-operator-builder
	- $(CONTAINER_TOOL) buildx build --push --platform=$(PLATFORMS) --tag ${IMG} -f Dockerfile.cross .
	- $(CONTAINER_TOOL) buildx rm jira-jit-rbac-operator-builder
	rm Dockerfile.cross
